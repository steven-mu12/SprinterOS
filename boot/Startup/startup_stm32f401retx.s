 ...
 * A portion of this software is done as a part of the SprinterOS project, by
 * STEVEN MU. The software is provided as-is, and if an issue arises, the author
...
.word _sidata		/* start add for init values of .data, this is in linker script */
.word _sdata		/* start add for .data, this is in linker script */
...
/* SPRINTEROS EDITS BEGIN HERE */
Reset_Handler:
  ldr   r0, =_estack	/* set SP to be the end of stack (= end of SRAM space) */
  mov   sp, r0

  ldr r0, =_sdata		/* this is SRAM */
  ldr r1, =_edata		/* this is SRAM */
  ldr r2, =_sidata		/* this is stored in FLASH */
  movs r3, #0
  b LoopCopyDataInit	/* copy data initializers from FLASH to SRAM for execution */

CopyDataInit:
  ldr r4, [r2, r3]		/* load R4 with what's at sidata + offset (R3) (all this is in FLASH)- this is 4 Bytes */
  str r4, [r0, r3]		/* store value of R4 at .data in SRAM, then read another 4 bytes */
  adds r3, r3, #4
  b	LoopCopyDataInit

LoopCopyDataInit:
  adds r4, r0, r3		/* we basically check if we read all of SRAM's worth of data from FLASH */
  cmp r4, r1			/* if not read another 4 bytes */
  bcc CopyDataInit

  ldr r2, =_sbss		/* zero out the BSS section word by word (almost the same procedure as above) */
  ldr r4, =_ebss
  movs r3, #0
  b LoopFillZerobss
...
/**
 * @ brief  When something bad happens, an unexpected IRQ is generated
 *         	we just enter an infinite loop so we can look via debugger
 * 			- During the task where we set up fault handlers, this will
 *			  be addressed in a better manner (SPRBOOT-10), but for now
 * 	    	  this handler will be the default go to for everything
*/
